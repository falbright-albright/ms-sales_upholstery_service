...
- [ ] Property/equipment coverage (tools, sewing machines, workshop
      contents)
- [ ] **Customer-property (bailee's) coverage** — furniture in your care,
      custody or control while being repaired/reupholstered — this is
      often the most relevant coverage for this specific trade
- [ ] Business interruption coverage, if appropriate for the business's
      risk tolerance
- [ ] Commercial vehicle insurance, if a vehicle is used to transport
      customer furniture (personal auto policies typically exclude
      business use)
- [ ] Workers' compensation (WSIB) obligations, if/when employees or
      subcontractors are hired

**Do not represent the business as insured on the website or verbally**
until an actual policy is in place and its coverage confirmed.
...
- Legal business name (if different from a working/trade name)
...
## 7. Gallery photo/video permission (confirm before launch)

`public/images/gallery/` and `public/videos/` now contain real project
...
brand signage visible in a couple of shots). **Before this site goes live,
confirm M.S. has the commercial client's permission to publish photos
that identify their business** — this wasn't something that could be
confirmed automatically when the photos were added to the site. If
permission hasn't been obtained, either get it, or crop/replace those
specific photos with ones that don't show identifying branding.
